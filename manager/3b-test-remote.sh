...
if ! kubectl get clusterqueue remote-cluster-queue -n kueue-system > /dev/null 2>&1; then
    ...
    print_error "- ClusterQueue: remote-cluster-queue"
    print_error "- LocalQueue: remote-queue"
    print_error "- MultiKueueCluster: remote-cluster"
    exit 1
fi

if ! kubectl get localqueue remote-queue -n multikueue-demo > /dev/null 2>&1; then
    print_error "Remote queue not found. Expected: remote-queue in multikueue-demo namespace."
    print_error "Please ensure remote configuration was applied properly."
    exit 1
fi
...
kubectl delete jobs --all -n multikueue-demo 2>/dev/null || true
kubectl --kubeconfig="$REMOTE_KUBECONFIG" delete jobs --all -n multikueue-demo 2>/dev/null || true
...
sed 's/name: multikueue-test-job/name: remote-test-job/g; s/queue-name: worker-queue/queue-name: remote-queue/g' sample-job.yaml > remote-test-job.yaml
...
while [ $COUNTER -lt $TIMEOUT ]; do
    ...
    if kubectl --kubeconfig="$REMOTE_KUBECONFIG" get job remote-test-job -n multikueue-demo 2>/dev/null | grep -q "remote-test-job"; then
...
run_cmd kubectl get jobs -n multikueue-demo -o wide

print_info "Job status on REMOTE cluster:"
run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" get jobs -n multikueue-demo -o wide

print_info "Waiting for job to start on remote cluster..."
run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" wait --for=condition=ready pod -l job-name=remote-test-job -n multikueue-demo --timeout=60s || true

# Get pod name on remote cluster - try multiple approaches
REMOTE_POD=""

# First, try to find pod using job-name label (works when job exists)
REMOTE_POD=$(kubectl --kubeconfig="$REMOTE_KUBECONFIG" get pods -n multikueue-demo -l job-name=remote-test-job -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
...
if [ -n "$REMOTE_POD" ]; then
    ...
    if [ "$POD_STATUS" = "Succeeded" ] || [ "$POD_STATUS" = "Failed" ]; then
        ...
        run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" logs -n multikueue-demo "$REMOTE_POD" || echo "No logs available"
    else
        print_info "Pod is running, streaming logs from remote cluster pod: $REMOTE_POD"
        run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" logs -n multikueue-demo "$REMOTE_POD" -f --tail=100 || true
    fi
...
JOB_COMPLETED=false
if kubectl --kubeconfig="$REMOTE_KUBECONFIG" get job remote-test-job -n multikueue-demo >/dev/null 2>&1; then
    if run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" wait --for=condition=complete job/remote-test-job -n multikueue-demo --timeout=120s; then
    ...
    if [ -n "$REMOTE_POD" ]; then
        POD_STATUS=$(kubectl --kubeconfig="$REMOTE_KUBECONFIG" get pod "$REMOTE_POD" -n multikueue-demo -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
...
run_cmd kubectl get workloads -n multikueue-demo -o wide

print_info "Remote cluster events:"
run_cmd kubectl --kubeconfig="$REMOTE_KUBECONFIG" get events -n multikueue-demo --sort-by='.lastTimestamp' | tail -10
